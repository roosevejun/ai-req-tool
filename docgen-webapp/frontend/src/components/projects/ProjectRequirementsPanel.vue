<template>
  <WorkspaceSection
    eyebrow="需求管理"
    title="需求管理中心"
    description="在当前项目下创建需求、跟踪需求状态，并把合适的需求快速送入 AI 需求工作台。"
  >
    <div class="split-grid">
      <section class="panel">
        <div class="panel-head">
          <div>
            <h4>新建需求</h4>
            <p class="muted">先写清需求标题和摘要，再决定优先级与状态。</p>
          </div>
        ...
        <textarea v-model="reqForm.summary" class="input textarea" placeholder="补充当前需求的核心目标、范围和背景。" />
        <div class="row">
      ...
      <section class="panel">
        <div class="panel-head">
          <div>
            ...
            <p class="muted">先选中需求，再决定进入工作台还是查看版本历史。</p>
          </div>
        </div>
        <table class="table">
          <thead>
          ...
          <tbody>
            <tr v-for="requirement in requirements" :key="requirement.id">
              ...
              <td class="ops">
                <button class="mini" @click="$emit('select-requirement', project.id, requirement.id)">查看</button>
                <button class="mini" @click="$emit('open-workbench', requirement.id)">AI 工作台</button>
                <button class="mini" @click="$emit('open-versions', requirement.id)">版本中心</button>
              </td>
            </tr>
            <tr v-if="requirements.length === 0">
              <td colspan="6" class="empty small">当前项目下还没有需求，建议先创建第一条需求。</td>
            </tr>
          </tbody>
        </table>
      </section>
    ...
    <section v-if="selectedRequirement" class="selected-panel">
      <div class="panel-head">
        <div>
          <p class="section-label">当前焦点需求</p>
        ...
        <div class="row compact">
          ...
          <button class="ghost mini" @click="$emit('open-versions', selectedRequirement.id)">查看版本中心</button>
        </div>
      </div>
      <div class="meta-grid">
      ...
      <p class="summary">{{ selectedRequirement.summary || '当前需求还没有补充摘要。建议先补背景和目标，再进入 AI 工作台。' }}</p>
    </section>
  </WorkspaceSection>
</template>
