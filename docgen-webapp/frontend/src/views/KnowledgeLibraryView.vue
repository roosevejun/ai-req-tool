    ...
    <div class="layout">
      <aside class="sidebar">
        <WorkspaceSection
          eyebrow="Scope"
          title="选择知识范围"
          description="先选项目，再按需切换到某条需求的知识文档。"
          :tint="true"
        >
          <div class="field">
            <label>项目</label>
          ...
          <div class="field">
            ...
            <select v-model="selectedRequirementId" class="input" :disabled="!selectedProjectId">
              ...
              <option v-for="item in requirements" :key="item.id" :value="String(item.id)">
                {{ item.title }}
              </option>
            </select>
          </div>
        ...
        <WorkspaceSection
          eyebrow="Filters"
          title="文档筛选"
          description="按状态和类型筛选当前范围内的知识文档。"
        >
      ...
      <main class="content">
        <EmptyWorkspaceState
          ...
          eyebrow="Knowledge Ready"
          title="先选择一个项目"
          description="知识库会按项目或需求范围展示文档状态、摘要、资源和重跑能力。"
        />
        ...
        <template v-else>
          <WorkspaceSection
            ...
            title="知识文档列表"
            :description="selectedRequirement ? `当前展示需求 #${selectedRequirement.id} 的知识文档。` : '当前展示项目级知识文档。'"
          >
            <template #actions>
              <div class="metrics">
              ...
              <article v-for="doc in filteredDocuments" :key="doc.id" class="document-card">
                <div class="document-head">
                  ...
                  <div class="doc-status">
                    ...
                    <StatusBadge
                      v-if="doc.latestTaskStatus"
                      :label="`任务：${knowledgeStatusLabel(doc.latestTaskStatus)}`"
                      :variant="statusVariant(doc.latestTaskStatus)"
                      small
                    />
                  </div>
                </div>
                ...
                <div class="actions">
                  ...
                  <button class="ghost" :disabled="reprocessingId === doc.id" @click="retryDocument(doc.id)">
                    {{ reprocessingId === doc.id ? '处理中...' : '重新处理' }}
                  </button>
                </div>
              </article>
            </div>
...
const quickFilters = [
  ...
  { key: 'FAILED' as const, label: '失败项' },
  { key: 'PENDING' as const, label: '待处理项' },
  { key: 'RECENT' as const, label: '最近新增' }
]
const filteredDocuments = computed(() => {
  const items = documents.value.filter((doc) => {
    const status = doc.status || doc.latestTaskStatus || ''
    if (statusFilter.value !== 'ALL' && normalizeStatus(status) !== statusFilter.value) {
      return false
    }
    if (typeFilter.value !== 'ALL' && normalizeType(doc.documentType) !== typeFilter.value) {
      return false
    }
    return true
  })
...
function normalizeType(value?: string) {
  ...
  if (['URL', 'TEXT', 'FILE'].includes(upper)) return upper
...
function documentTypeLabel(value?: string) {
  ...
  if (normalized === 'URL') return '网站链接'
  if (normalized === 'TEXT') return '文本资料'
  if (normalized === 'FILE') return '文件资料'
  return value || '未知类型'
}

function knowledgeStatusLabel(value?: string) {
  const normalized = normalizeStatus(value)
  if (normalized === 'READY') return '处理完成'
...
function statusVariant(value?: string) {
  ...
  if (normalized === 'PROCESSING') return 'ai'
  if (normalized === 'PENDING') return 'warning'
  return 'neutral'
}

function applyQuickFilter(filter: 'ALL' | 'FAILED' | 'PENDING' | 'RECENT') {
...
function syncQuickFilterFromStatus() {
  if (statusFilter.value === 'FAILED') {
    activeQuickFilter.value = 'FAILED'
    return
  }
  if (statusFilter.value === 'PENDING' || statusFilter.value === 'PROCESSING') {
...
async function loadDocuments() {
  ...
  error.value = ''
  try {
    const path = selectedRequirementId.value
      ? `/api/knowledge-documents/requirements/${selectedRequirementId.value}`
      : `/api/knowledge-documents/projects/${selectedProjectId.value}`
    const res = await axios.get<ApiResponse<KnowledgeDocumentListItem[]>>(path)
    documents.value = res.data.data || []
  } catch (e: any) {
    error.value = e?.response?.data?.message || e?.message || '加载知识文档失败。'
  } finally {
    loading.value = false
  }
}
...
async function openDetail(documentId: number) {
  ...
  await router.replace({
    ...
    query: {
      ...
      requirementId: selectedRequirementId.value || undefined,
...
async function retryDocument(documentId: number) {
  ...
  try {
    ...
    await loadDocuments()
    if (detail.value?.document.id === documentId) {
...
async function reprocessDetail() {
  ...
  try {
    ...
    detailReprocessing.value = false
  }
}
...
function closeDetail() {
  ...
  void router.replace({ path: '/knowledge', query: nextQuery })
}
...
async function applyRouteSelection() {
  ...
  if (queryProjectId && selectedProjectId.value !== queryProjectId) {
    ...
    await loadRequirements(queryProjectId)
  }

  if (queryRequirementId) {
    selectedRequirementId.value = queryRequirementId
  }

  if (queryDocumentId > 0) {
    if (!detailVisible.value || detail.value?.document.id !== queryDocumentId) {
      await openDetail(queryDocumentId)
    }
  }
}
